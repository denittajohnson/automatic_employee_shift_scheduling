...
<table id="example" class="table table-striped table-bordered dataTable no-footer" cellspacing="0"  role="grid" aria-describedby="example_info" >
		  ...
		  include("../connection.php");
			...
			{
			$a_id=$_REQUEST['a_id'];
			mysqli_query($con,"update $table set status='accepted' where id='$a_id'");
...
<th>Action</th>
  ...
  // $i=0;
   echo "<tbody>";
	...
	{		
 	$result = mysqli_query($con,"SELECT * FROM $table where department='$_SESSION[did]'");
	}
	else{
		$result = mysqli_query($con,"SELECT e.* FROM employee e JOIN department d ON e.department = d.id WHERE d.cid = '$_SESSION[uid]'");
	}
...
$row2 =mysqli_fetch_array($result2);
			...
			<td>
				<a href='?a_id=$id' class='btn btn-success'>Accept</a>
