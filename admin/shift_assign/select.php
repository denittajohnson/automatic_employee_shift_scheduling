...
<?php
	...
	if($_REQUEST['id']==1)
	{
		echo "<center><p>Shift assignments successfully updated.</p></center>";
	}
	elseif($_REQUEST['id']==2)
	{
		echo "<center><p>No new assignments needed.</p></center>";
	}
   ...
   echo "<tbody>";
 	...
 	$result = mysqli_query($con,"SELECT * FROM $table ");
		...
		for($k=0;$k<$i;$k++)
		{
			...
			if($k==1)
			{
			  $sql2 = "select *  from shift where id='$row[$k]' ";
    $result2 = mysqli_query($con, $sql2) or die("Error in Selecting " . mysqli_error($connection));
$row2 =mysqli_fetch_array($result2);
		
		

			echo "<td >  $row2[shift_name]</td>";
...
$row2 =mysqli_fetch_array($result2);
			...
			elseif($k==4)
			{
			  $sql2 = "select *  from department_head where id='$row[$k]' ";
    $result2 = mysqli_query($con, $sql2) or die("Error in Selecting " . mysqli_error($connection));
$row2 =mysqli_fetch_array($result2);
